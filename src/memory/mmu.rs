...
impl fmt::Display for TranslationDescriptor {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        let (size, unit) = crate::common::size_human_readable_ceil(size);
...
impl<const NUM_SPECIAL_RANGES: usize> KernelVirtualLayout<{ NUM_SPECIAL_RANGES }> {
    ...
    pub fn virt_addr_properties(
        ...
        debug!("virt_addr_properties execution");
        if virt_addr > self.max_virt_addr_inclusive {
            debug!("Address out of range");
            return Err("Address out of range");
        }
        debug!("self.inner.iter() !{}", virt_addr);
            ...
            if (i.virtual_range)().contains(&virt_addr) {
                let output_addr = match i.physical_range_translation {
                    ...
                    Translation::Offset(a) => {
                        debug!("convert");
                        a + (virt_addr - (i.virtual_range)().start())
                    }
                };
