...
impl memory::mmu::interface::MMU for MemoryManagementUnit {
    unsafe fn enable_mmu_and_caching(&self) -> Result<(), MMUEnableError> {
        ...
        debug!("Prepare the memory attribute indirection register.");
        self.set_up_mair();

        debug!("Populate translation tables");
        let res = unsafe {
        ...
        let phys_base_address = KERNEL_TABLES.lock(|t| t.phys_base_address());

        debug!(
            "Set the Translation Table Base Register {:#b}",
            phys_base_address
        );
        TTBR0_EL1.set_baddr(phys_base_address);

        debug!("configure_translation_control");
        self.configure_translation_control();

        // Switch the MMU on.
        //
        // First, force all previous changes to be seen before the MMU is enabled.
        debug!("force all previous changes to be seen before the MMU is enabled");
        barrier::isb(barrier::SY);

        debug!("Enable the MMU and turn on data and instruction caching");
        SCTLR_EL1.modify(SCTLR_EL1::M::Enable + SCTLR_EL1::C::Cacheable + SCTLR_EL1::I::Cacheable);

        debug!("EForce MMU init to complete before next instruction.");
        barrier::isb(barrier::SY);
